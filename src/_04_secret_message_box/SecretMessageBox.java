...
public class SecretMessageBox {
	...
	public static void main(String[] args) {
		...
		String password = "1215226";
		// 2. Using a pop-up, ask the first person for a secret message and store it in
		// a variable
		String message = JOptionPane.showInputDialog("Place tho message.");
		// 3. Now use a pop-up to tell the NEXT user that they can only see the secret
		// message
		// if they can guess the passcode
		String thought = JOptionPane
				.showInputDialog("'Tis what tho password?");
		// 4. If their guess matches the password, show them the message
		if (thought.equals(password)) {
			JOptionPane.showMessageDialog(null, message);
		}
			// 5. If the password does not match,
else {
	JOptionPane.showMessageDialog(null, "You do not deserve the message!");
}
	}
}